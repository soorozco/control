...
# Tab 1: Subir JSON
with tabs[0]:
    st.header("Subir archivos JSON y extraer datos")
    uploaded_jsons = st.file_uploader(
        "Selecciona uno o más archivos JSON", 
        type=["json"], 
        key="json_upload",
    ...
    if uploaded_jsons:
        ...
        for uploaded_json in uploaded_jsons:
            try:
                ...
                if not json_content:
                    errores.append(f"❌ {uploaded_json.name}: Formato inválido")
                    continue
                ...
                registro_json = {
                    ...
                    "pasos": json.dumps(json_content.get("Desarrollo del Proceso", {}).get("table", [])),
                ...
                with engine.connect() as conn:
                    existing = pd.read_sql(
                        f"SELECT * FROM documentos WHERE codigo = '{registro_json['codigo']}'", 
                        conn
                    )
                    if not existing.empty:
                        errores.append(f"⚠️ {uploaded_json.name}: Código {registro_json['codigo']} ya existe")
                        continue

                # Procesar pasos y autorizaciones
                ...
                formatos_detectados.update(formatos)
                ...
                # Registrar personal de autorizaciones
                try:
                    ...
                    errores.append(f"⚠️ {uploaded_json.name}: Error en autorizaciones - {str(e)}")
                
                documentos_procesados.append(registro_json['codigo'])
                total_procesados += 1

            except Exception as e:
                errores.append(f"❌ {uploaded_json.name}: Error crítico - {str(e)}")
        
        # Mostrar resumen de procesamiento
        st.success(f"**Procesamiento completo:** {total_procesados}/{len(uploaded_jsons)} archivos exitosos")
        
        if documentos_procesados:
            st.markdown(f"**Documentos insertados:** {', '.join(documentos_procesados)}")
        
        # Mostrar formatos detectados
        if formatos_detectados:
            st.subheader("🔍 Formatos detectados en los documentos")
            cols = st.columns(3)
            for i, formato in enumerate(sorted(formatos_detectados)):
                with cols[i % 3]:
                    st.info(f"**Formato:** {formato}")
            st.warning("Los formatos detectados deben registrarse manualmente en la pestaña 'Control de Registros'")
        ...
        st.divider()
        if st.button("🔄 Actualizar Control de Documentos", key="update_docs_btn"):
            st.cache_data.clear()
            st.success("Base de datos y vista de documentos actualizadas correctamente")
            st.rerun()
...
with tabs[4]:
    ...
    grid_response = None  # Inicializar la variable
    
    if not personal_df.empty:
        # Preprocesar los datos para que sean compatibles con AgGrid
        personal_df = personal_df.astype(str)
